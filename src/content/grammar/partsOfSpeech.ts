...
export const partsOfSpeech: GrammarCategory = {
  ...
  topics: [
    ...
    },
    {
      ...
      theory:
        'Коли кілька прикметників стоять перед іменником, вони йдуть у типовому порядку: `opinion → size → age → shape → colour → origin → material → purpose + noun` (думка → розмір → вік → форма → колір → походження → матеріал → призначення; напр. `a beautiful small old round black Italian wooden dining table` — хоча на практиці рідко використовують стільки одразу). Короткі прикметники (1 склад, деякі 2-складові) утворюють порівняльний ступінь через -er/-est (`tall → taller → tallest`), довші — через more/most (`expensive → more expensive → most expensive`). Подвійний порівняльний зворот `the more..., the better/more...` виражає пропорційну залежність двох явищ: `The more you practice, the better you get.`',
      examples: [
