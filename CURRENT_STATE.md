...
- **In-app reminder banner** (`src/components/ReminderBanner/`, Task 5a) — `useReminders()` (`src/hooks/useReminders.ts`) polls the schedule once a minute and surfaces two kinds of dismissible nudges, rendered globally in the `App` shell above `<main>` so they show on every page: an "English unblocked" nudge fired on the blocked->available transition of `getEnglishSeparationStatus()` (`shouldFireUnblockNudge()`), and reminders for any labeled schedule occurrence (recurring or exception) starting within `UPCOMING_EVENT_LEAD_MINUTES` (60) or already in progress (`getUpcomingEventReminders()`) — this is what covers the Thursday "Mi Casa, Graz" exchange, generically rather than as a hardcoded case. Dismissals persist per reminder id in `localStorage` so a closed banner doesn't resurface on reload. Purely a nudge — nothing gates Vocabulary/Shadowing (see `DECISIONS.md`). Web Push (VAPID) is deferred to Task 5b.
- **Test infra** — 49 tests passing across 15 files. `src/setupTests.ts` polyfills three jsdom gaps: `localStorage`, IndexedDB (`fake-indexeddb/auto`), and `SpeechSynthesisUtterance`. All new UI text across every module is localized (uk/ru) through the i18n system.

## Not built yet

See [`ROADMAP.md`](./ROADMAP.md) for the task-by-task checklist and scope of each remaining piece (Web Push, Shadowing Lab, Error Journal, Dashboard, Export/Import). Additional IndexedDB stores get added incrementally, one per owning task, via schema version bumps in `src/services/db/schema.ts` — not all upfront.
